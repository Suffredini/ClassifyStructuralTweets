...
public class BuildClassifierConstantNumberOfNSTweets {
    /* START Parameter */
    ...
    private static final int maxTweets = 5;
    ...
    //private static double weightC0 = 0.1; // weight of class C0, weight for class C1 autogenerated (fatti vari esperimenti e con 0.3 si ha maggior fscore per svm)
    ...
    public static void main(String[] args) throws IOException, Exception {
        ...
        //for(double w=0.3; w<=0.7; w+=0.1){               
            ...
            if(maxFscore < average(fscoreFold)){  
                ...
                switch(selectedClassifier){                
                    case 1:                
                        printParameter = "MaxTweet:"+maxTweets+" - W_non-structural:";                        
                        break;

                    case 3:                
                        printParameter = "MaxTweet:"+maxTweets+" - k: " + kValue;
                        selectedK = kValue;
                        break; 
                        
                    default:
                         printParameter = "MaxTweet:"+maxTweets;
                } 
        ...
        exportClassifier(classifier,classifierName+"(Exp.3)");
    ...
    private static void importTweets(String path) throws Exception {
        ...
        int countInstances, indexAttribute;
       ...
       // data.setClassIndex(data.numAttributes() - 1); // Questo è attributo key
        ...
        Instances tmpInstances = new Instances(dataset, 0);
        ...
        for(String word: keyWord){
            ...
            tmpInstances.clear();
            int structCount = 0;
            for(Instance tmpInst: keywordInstances){
                //System.out.println(tmpInst.stringValue(indexAttribute));
                if(tmpInst.stringValue(indexAttribute).equals("Structural")){
                    if(structCount<14){
                        structCount++;
                        // Aggiungo tutti gli structural
                        tmpInstances.add(tmpInst);
                    }
                } else if(countInstances < maxTweets){
                    // Aggiungo non structural finchè non raggiungo il massimo stabilito
                    tmpInstances.add(tmpInst);
                    countInstances++;
                }
            }
