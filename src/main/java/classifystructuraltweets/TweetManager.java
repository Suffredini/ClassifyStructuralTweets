...
public class TweetManager {
	 ...
	 * @throws Exception
	 */
	private static String getURLResponse(String username, String since, String until, String position, String querySearch, String scrollCursor) throws Exception {
		String appendQuery = "";
		if (username != null) {
	...
	public static List<Tweet> getTweets(TwitterCriteria criteria) {
		...
		try {
			...
			outerLace: while (true) {
				JSONObject json = new JSONObject(getURLResponse(criteria.getUsername(), criteria.getSince(), criteria.getUntil(), criteria.getPosition(), criteria.getQuerySearch(), refreshCursor));
